...
const Usuario = () => {
  ...
  const fetchUsuarios = async () => {
    try {
      // Buscar usuários de cada tipo e extrair a lista corretamente
  ...
  const handleSave = async () => {
    try {
      ...
      const updatedUsuario = response.data.user; // Ajuste aqui para pegar o usuário atualizado
      setUsuarios(
  ...
  const renderUserTable = (tipo) => (
    <Table striped bordered hover style={{ textAlign: "center" }}>
      ...
      <tbody>
        {usuarios
          .filter((usuario) => usuario.tipo === tipo) // Filtra pelo tipo correto
          .map((usuario) => (
            <tr key={usuario._id}>
              <td>{usuario._id}</td>
              <td>
